...
export default function DashboardKPI() {
  ...
  useEffect(() => {
    fetchDashboard().then(r => setKpi(r.data)).catch(e => setError(e?.response?.data?.error || e.message));
  }, []);
  ...
  // ⚠️ Tu as demandé Occupation = (Jtravail - Jutilisation) / Jtravail ⇒ c’est la part LIBRE.
  // Pour “représenter comme la maquette” (part Occupé mise en avant), on affiche:
  // Occupé = 100 - KPI, Libre = KPI.
  ...
  return (
    <Box sx={{ maxWidth: 1440, mx: "auto", p: { xs: 1.5, md: 3 } }}>
      <Grid container spacing={2.25}>
        ...
        <Grid item xs={12} md={5}>
          <CardShell>
            ...
            <Grid container spacing={2}>
              <Grid item xs={6}><KPIBox label="ACHÉVÉS" value={kpi.acheves?.demandes ?? 0} unit="demandes" />
                <Typography variant="body2" color="text.secondary">{kpi.acheves?.echantillons ?? 0} échantillons</Typography>
              </Grid>
              <Grid item xs={6}><KPIBox label="EN COURS" value={kpi.enCours?.demandes ?? 0} unit="demandes" />
        ...
        {/* Cartes Durées (comme sur la maquette) */}
        ...
        {/* Bloc planification + bande “Respect des délais” comme la maquette */}
        <Grid item xs={12}>
          <CardShell>
            <Stack direction="row" spacing={1.25} alignItems="center" sx={{ mb: 1 }}>
              <Box sx={{ width: 32, height: 32, borderRadius: "50%", bgcolor: "primary.light", color: "primary.main", display: "grid", placeItems: "center" }}>
            ...
            <Box sx={{ height: 110, border: "1px dashed", borderColor: "divider", borderRadius: 1.25, mb: 1.25, display: "grid", placeItems: "center", color: "text.disabled" }}>
              <Typography variant="caption">Espace planning (clients / modèles / type d’essai)</Typography>
            </Box>
